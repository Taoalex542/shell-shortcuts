...
void is_neg_float_e_gmaj(long long *i, double *f)
{
    if ((*i) < 0) {
        (*i) = (*i) * -1;
        (*f) = (*f) * -1;
...
void put_zero_e_f_gmaj(int len, int temp, int size, double f)
{
    int test = 0;
...
int my_put_e_gmaj(double f, int z)
{
    ...
    long long d = 0;
    long long a = 0;
    int l = get_size_e_f_gmaj(z);

    my_put_nbr(((i % (l * 10)) - (i % l)) / l);
    my_putchar('.');
    is_neg_float_e_gmaj(&i, &f);
    d = ((i / 10 % l) - i / l * l / 10);
    a = ((i % l) - d * 10);
    put_zero_e_f_gmaj(l, d, l, f);
    if (a > 5)
        d = d + 1;
    my_put_nbr(d);
    for (int k = z; f == 0 && k - 1 > 0; k = k - 1)
        my_putchar('0');
    return print_power_gmaj(z, f, 1);
}
