...
void is_neg_float_e(long long *i, double *f)
{
    if ((*i) < 0) {
        (*i) = (*i) * -1;
        (*f) = (*f) * -1;
...
void put_zero_e_f(int len, int temp, int size, double f)
{
    int test = 0;
